...
export default function BKPlaceholderPage({ params }: { params: { slug: string[] } }) {
  ...
  return (
    <main className="space-y-6">
      <div className="bg-white/40 backdrop-blur-2xl border border-white/50 rounded-3xl p-8 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)]">
        <div className="text-xs font-bold text-slate-500">BK</div>
      ...
      <div className="bg-white/40 backdrop-blur-2xl border border-white/50 rounded-3xl p-6 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)]">
        ...
        <div className="text-sm text-slate-600 mt-1">/dashboards/bk/{params.slug.join("/")}</div>

        <div className="mt-6 flex flex-wrap gap-2">
          <Link href="/dashboards/bk" className="px-3 py-2 rounded-xl bg-white/60 border border-white/70 text-sm font-semibold text-slate-800 hover:bg-white/80 transition-colors">
            Dashboard BK
          </Link>
